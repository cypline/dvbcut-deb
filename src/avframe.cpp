...
avframe::avframe(AVFrame *src, AVCodecContext *ctx) : f(0),tobefreed(0)
  ...
  img_convert_ctx=sws_getContext(w, h, pix_fmt, 
                                 w, h, AV_PIX_FMT_RGB32, SWS_BICUBIC,
                                 NULL, NULL, NULL);
#endif
  }
...
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(57, 0, 0)
  uint8_t *rgbbuffer = (uint8_t*)malloc(av_image_get_buffer_size(AV_PIX_FMT_RGB32, w, h, 1));
#else
  uint8_t *rgbbuffer=(uint8_t*)malloc(avpicture_get_size(AV_PIX_FMT_RGB32, w, h));
#endif
...
#else
  img_convert((AVPicture *)avframergb, AV_PIX_FMT_RGB32, (AVPicture*)f, pix_fmt, w, h);
#endif

  QImage im(rgbbuffer, w, h, 4*w, QImage::Format_RGB32, ::free, rgbbuffer);
